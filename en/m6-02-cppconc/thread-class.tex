...
\begin{frame}[t,fragile]{Simplifying with jthread}
\begin{itemize}
...
\begin{lstlisting}
void example() {
  std::jthread t1{f}; // Thread running task f
  std::jthread t2; // Empty thread
...
\begin{frame}[t,fragile]{Operations on current thread}
\begin{itemize}
