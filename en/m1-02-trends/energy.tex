...
\begin{itemize}
  \item Two different systems (\bulletmark{A} y \bulletmark{B}).
    \begin{itemize}
      \item \bulletmark{A} consumes 20\% more power than \bulletmark{B}.
      \item \bulletmark{A} runs a task in 70\% of \bulletmark{B} time.
      \item Which has a lower cost?
    \end{itemize}

  \mode<presentation>{\pause\vfill}
  \item The adequate metric for comparison is \bulletgood{Energy}.
    \begin{itemize}
      \item $E(B) = P(B) \cdot t(B)$
      \item $E(A) = 1.2 \cdot P(B) \cdot 0.7 \cdot t(B) = 0.84 \cdot E(B)$
      \item System \bulletmark{A} uses 84\% of \bulletmark{B} energy.
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item In CMOS technology, \bulletmark{energy consumption} is derived from
        \bulletmark{transistors switching}.

  \mode<presentation>{\pause\vfill}
  \item \bulletenum{Dynamic energy}:
    \begin{itemize}
...
\begin{itemize}
  \item Trends over time:
    \begin{itemize}
      ...
      \item \textemph{Net effect}: Growth in power comsumption and enegery.
    \end{itemize}

  \mode<presentation>{\vfill\pause}
  \item \textgood{Power Comsumption}
    \begin{itemize}
      \item Intel 80386 (1986): 2W.
      \item Intel Core i7-6700K (2015): 91W.
      \item Intel Core i7-12700K (2021): 125W.
      \item \textbad{Aproaching limits of cooling}.
    \end{itemize}
