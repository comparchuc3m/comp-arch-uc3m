...
\paragraph{Compiling the debug version}
\begin{itemize}
  \item Generates a project configuration in directory \cppid{mandel-debug}
...
\paragraph{Compiling the release version}
\begin{itemize}
  \item Generates a project configuration in directory \cppid{mandel-release}
        from the source files in directory \cppid{mandel}.
        Use this option with the variable \cppkey{DCMAKE\_BUILD\_TYPE=Release},
        to use the release compiler options.
\begin{lstlisting}[style=terminal]
cmake -S mandel -B mandel-release -DCMAKE_BUILD_TYPE=Release
\end{lstlisting}
\end{itemize}
...
The \cppkey{perf} tool offers several sub-commands.
