\clearpage
\subsection{Exercise: Matrix product}
...
data is stored by rows. The \cppkey{[]} operator is overloaded so
...
listing~\ref{lst:create-matrix}) which allows you to create a matrix with random
values from a number of rows, a number of columns and a
random number generator. The function fills each cell with a random value
...
The example includes two fully implemented versions of the matrix product that
...
must be developed from the function \cppid{matrix\_multiply\_seq()}.

\subsubsection{Parallel version based on integer index}

The simplest way to parallelize a loop is using the version of
\cppid{tbb::parallel\_for()} based on an integer index (see
listing~\ref{lst:parallel-for-idx}).
In this version the lower index and upper index are passed (which must be
of the same type) and a lambda expression that takes as sole parameter an
integer value.

\begin{lstlisting}[label={lst:parallel-for-idx},
caption={Using parallel for loop based on index}]
...
The TBB library is responsible for distributing iterations among available
threads.

Use this type of call to modify the function
\cppid{matrix\_multiply\_par()} and measure the achieved speedup.

\subsubsection{Parallel version based on one-dimensional block}

Another way to parallelize a loop is using the version of
\cppid{tbb::parallel\_for()} based on one-dimensional block ranges
(\cppid{tbb::blocked\_range<T>}) (see listing~\ref{lst:parallel-for-rng1}).

In this version the range is passed as a value of type
\cppid{tbb::blocked\_range<T>} and a lambda expression that takes as sole
parameter a value of type \cppid{blocked\_range<T>}.

\begin{lstlisting}[label={lst:parallel-for-rng1},
caption={Using parallel for loop based on 1D range}]
...
The TBB library is responsible for dividing the range into smaller subranges up to
a certain level of granularity. When a range is no longer divided it is passed to
the lambda.

Use this type of call to modify the function
\cppid{matrix\_multiply\_par\_range()} and measure the achieved speedup.

\subsubsection{Parallel version based on two-dimensional block}

Another similar way to parallelize a loop is using the version of
\cppid{tbb::parallel\_for()} based on two-dimensional block ranges
(\cppid{tbb::blocked\_range2d<T>}) (see listing~\ref{lst:parallel-for-rng2}).

In this version the range is passed as a value of type
\cppid{tbb::blocked\_range2d<T>} and a lambda expression that takes as sole
parameter a value of type \cppid{blocked\_range2d<T>}.

\begin{lstlisting}[label={lst:parallel-for-rng2},
caption={Using parallel for loop based on 2D range}]
...
both dimensions up to a certain level of granularity. When a range is no longer
