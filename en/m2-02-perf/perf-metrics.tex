...
\mode<presentation>{\pause\vfill}
\begin{itemize}
  \item \textmark{Y} runs \textemph{n} \textgood{times faster} than \textmark{X}.
\end{itemize}
\begin{block}{Speedup}
\begin{math}
n=\frac{T(x)}{T(y)}=
...
\begin{itemize}
  \item The \textbad{only} \textemph{reliable} metric for comparing computer performance
  ...
  \item \textgood{Execution time}.
    \begin{itemize}
      ...
      \item \textmark{CPU time}: Time the CPU has been busy.
        \begin{itemize}
          \item With \textgood{a single processor} in use, it is \textemph{usually lower}
                than \textmark{response time}.
          \item With \textgood{multiple processors} in use, it is \textemph{usually higher}
                than \textmark{response time}.
        \end{itemize}
    \end{itemize}
\end{itemize}
