...
\begin{frame}[t]{C++ y consistencia de memoria}
\begin{itemize}
  \item C++ define un \textmark{modelo de concurrencia} propio como parte del propio lenguaje.
...
\begin{frame}[t]{Objetos y posiciones de memoria}
\begin{itemize}
  ...
  \mode<presentation>{\vfill\pause}
  \item \textgood{Posición de memoria}: Es un objeto de un \textmark{tipo
        escalar} o una secuencia de \textmark{campos de bits} adyacentes.

    \begin{itemize}
      \item \textgood{Tipo escalar}:
            Tipos aritméticos, tipos enumerados, tipos puntero.

      \item \textgood{Campo de bits}:
            Dato miembro entero con tamaño explícito en bits.
    \end{itemize}
...
\begin{frame}[t]{Ordenamiento y condiciones de carrera}
\begin{itemize}
...
\begin{frame}[t]{Orden de modificación}
\begin{itemize}
