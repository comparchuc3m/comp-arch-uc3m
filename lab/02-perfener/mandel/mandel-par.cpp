...
class portable_binary_bitmap {
public:
  using Size=std::size_t;
  portable_binary_bitmap(std::ostream& ostr, Size width, Size height)
      : _ostr (ostr)
  ...
  public:
    ...
    InterlacedCanvas(portable_binary_bitmap & pbm, Size yStart, Size increment)
        noexcept
        : _pbm (pbm)
          , _yStart (yStart)
...
public:
  ...
  using Line = typename portable_binary_bitmap::Line;
  using Size = std::size_t;

  ComplexPlaneCalculator(const std::complex<NUMBER_TYPE>& cFirst,
                         const std::complex<NUMBER_TYPE>& cLast,
                         portable_binary_bitmap::InterlacedCanvas& canvas, Functor f) noexcept
      : _cFirst(cFirst)
        , _cLast(cLast)
        , _canvas(canvas)
        , _f(f) {
...
private:
  ...
  portable_binary_bitmap::InterlacedCanvas _canvas;
  Functor _f;
};
...
int main(int argc, char** argv) {
  ...
  portable_binary_bitmap pbm(std::cout, n, n);
